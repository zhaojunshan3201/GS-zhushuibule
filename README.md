...
`单井井史` 支持 PPT/PPTX 批量导入。导入时后端按文件名识别井号，通过 Windows 桌面版 Microsoft PowerPoint COM 自动化逐页导出 PNG，并按页序插入对应井的富文本正文；原始 PPT/PPTX 同时保留为当前井史附件。

单次选择超过 20 个文件或 48MB 时，前端会提示并自动拆分为串行批次；单文件超过 48MB 会阻止本次导入。相同井号以本次选择中最后一个文件为准，前序文件记为“覆盖跳过”。导入前必须选择作业区。

井史目录按“作业区 → 井号”显示为二级可折叠菜单，全部作业区默认展开；未填写作业区的档案归入“未分配作业区”。井号的打开、删除及选中操作保持不变。

上传的 PPT/PPTX 原文件会自动保存到 `uploads/well-history-source/`，逐页导出的 PNG 保存到 `uploads/well-history/`。数据库保存井号、附件路径、原始文件信息、富文本正文及版本记录等元数据。

井史查看区顶部提供作业区、区块、井号筛选，以及查询、保存编辑结果、下载 PDF 操作。富文本正文中的 PNG、文字和表格均可继续编辑，并可导出为 PDF。

## 侧边栏 Logo

“采油管理部”账号可在“系统设置 → 基础配置”中上传侧边栏 Logo。仅支持本地图片文件，单文件最大 5MB；上传成功后侧边栏立即更新，无需刷新页面。未上传 Logo 时，系统继续显示默认侧边栏标识；连续选择多张图片时仅保留最后一次选择。
...
沿用现有系统设置权限，仅 `ADMIN` 角色可在“系统设置 → 首页储量看板颜色”修改配色。页面提供五个可视化取色器，可选择黄色、绿色等任意颜色，默认值如下：
...
“恢复默认色”只把五个取色器恢复为默认值，不会立即修改全局配置；管理员仍需点击“保存颜色”确认。保存后的颜色在用户刷新或重新进入首页时生效，配置读取失败时首页自动使用上述默认色。
...
列表页的“首页、上一页、下一页、尾页”会随当前主题使用对应主色：默认主题为深蓝色、中石油红为红色、极简企业白为企业蓝、墨绿鎏金为墨绿色，避免使用与主题不协调的高饱和链接蓝。
