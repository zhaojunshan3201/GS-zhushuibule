...
test("system settings edits, restores, and atomically saves reserve chart colors", () => {
  ...
  assert.match(settingsSource, /const reserveChartColorsSaveInFlightRef = useRef\(false\)/);
  assert.match(settingsSource, /const \[isSavingReserveChartColors, setIsSavingReserveChartColors\] = useState\(false\)/);
  ...
  assert.match(saveHandler, /await axios\.post\("\/api\/config", \{[\s\S]*?key: HOME_RESERVE_CHART_COLORS_CONFIG_KEY,[\s\S]*?value: serializedReserveChartColors,[\s\S]*?\}\)/);
  ...
  assert.equal((saveHandler.match(/serializeHomeReserveChartColors\(reserveChartColors\)/g) || []).length, 1);
  const guardStart = saveHandler.indexOf("if (reserveChartColorsSaveInFlightRef.current) return;");
  const acquireStart = saveHandler.indexOf("reserveChartColorsSaveInFlightRef.current = true;");
  const savingStart = saveHandler.indexOf("setIsSavingReserveChartColors(true);");
  const snapshotStart = saveHandler.indexOf("const serializedReserveChartColors = serializeHomeReserveChartColors(reserveChartColors);");
  const tryStart = saveHandler.indexOf("try {");
  const postStart = saveHandler.indexOf('await axios.post("/api/config"');
  assert.ok(guardStart >= 0 && acquireStart > guardStart && savingStart > acquireStart && snapshotStart > savingStart && tryStart > snapshotStart && postStart > tryStart);
  ...
  const finallyStart = saveHandler.indexOf("finally", catchStart);
  assert.ok(catchStart > postStart && finallyStart > catchStart);
  const successSource = saveHandler.slice(tryStart, catchStart);
  assert.match(successSource, /setError\(""\)/);
  assert.match(successSource, /setMessage\(SETTINGS_TEXT\.chartColorsSaved\)/);
  assert.ok(successSource.indexOf('setError("")') < successSource.indexOf("setMessage(SETTINGS_TEXT.chartColorsSaved)"));
  const catchSource = saveHandler.slice(catchStart, finallyStart);
  assert.match(catchSource, /setMessage\(""\)/);
  assert.match(catchSource, /setError\(SETTINGS_TEXT\.chartColorsSaveFailed\)/);
  assert.ok(catchSource.indexOf('setMessage("")') < catchSource.indexOf("setError(SETTINGS_TEXT.chartColorsSaveFailed)"));
  assert.doesNotMatch(catchSource, /setReserveChartColors|loadSettings|setMessage\(SETTINGS_TEXT\.chartColorsSaved\)|axios\./);
  const finallySource = saveHandler.slice(finallyStart);
  assert.match(finallySource, /reserveChartColorsSaveInFlightRef\.current = false/);
  assert.match(finallySource, /setIsSavingReserveChartColors\(false\)/);
  ...
  assert.match(colorSectionSource, /disabled=\{isSavingReserveChartColors\}/);
  assert.match(colorSectionSource, /disabled:cursor-not-allowed/);
  assert.match(colorSectionSource, /disabled:opacity-50/);
  assert.match(colorSectionSource, /isSavingReserveChartColors \? "保存中\.\.\." : SETTINGS_TEXT\.saveChartColors/);
